...
Stop.find_each do |stop|
  url = URI("https://api.winnipegtransit.com/v3/stops/#{stop.key}/schedule.json?api-key=aaitNO7SFcfMFhmbrg-u&start=2024-03-03T10:00:00&end=11:00:00&route=BLUE,650,642,691,694,47,16,18,31")
  response = Net::HTTP.get(url)
  result = JSON.parse(response)
  ...
  puts "Stop key:#{stop.key}, name:#{stop.name} "
  sleep(30) # pause 1s
end
